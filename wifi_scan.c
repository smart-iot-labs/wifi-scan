...
// public interface - initialize the library for wireless interface (e.g. wlan0). NULL value
//                    means error trying to initialize the library
struct wifi_scan *wifi_scan_init(const char *interface); 

// allocate memory, set initial values, etc.
bool init_netlink_channel(struct netlink_channel *channel, const char *interface);
// create netlink sockets for generic netlink
bool init_netlink_socket(struct netlink_channel *channel);
...
bool parse_CTRL_ATTR_MCAST_GROUPS(struct nlattr *nested, struct netlink_channel *channel);

// subscribes channel to multicast group scan using scan group id 
bool subscribe_NL80211_MULTICAST_GROUP_SCAN(struct netlink_channel *channel, uint32_t scan_group_id);
...
bool read_past_notifications(struct netlink_channel *notifications);
// go non-blocking 
bool set_channel_non_blocking(struct netlink_channel *channel);
// go back blocking
bool set_channel_blocking(struct netlink_channel *channel);
...
bool wait_for_new_scan_results(struct netlink_channel *notifications);
...
bool send_nl_message(struct nlmsghdr *nlh, struct netlink_channel *channel);
...
struct wifi_scan *wifi_scan_init(const char *interface)
{
  ...
  if(interface != NULL)
  {
    wifi = malloc(sizeof(struct wifi_scan));
    bool initDone = false;
    
    if(wifi != NULL)
    {
      initDone = init_netlink_channel(&wifi->notification_channel, interface);
      ...
      if(initDone)
      {
        ...
        if(get_family_and_scan_ids(&wifi->notification_channel) != -1)
        {
          ...
          {
            initDone = init_netlink_channel(&wifi->command_channel, interface);
            if(initDone)
            {
              wifi->command_channel.nl80211_id = wifi->notification_channel.nl80211_id;
              initDone = subscribe_NL80211_MULTICAST_GROUP_SCAN(&wifi->notification_channel, family_context.id_NL80211_MULTICAST_GROUP_SCAN);
            }
          }
          else
          {
            initDone = false;
            trace_error("No scan multicast group in generic netlink nl80211\n");
          }
        }
        else
        {
          initDone = false;
          perror("GetFamilyAndScanId failed");
        }
      }

      if(!initDone && wifi != NULL)
      {
        free(wifi);
        wifi = NULL;
      }
    ...
    {
      trace_error("Insufficient memory - malloc(sizeof(struct wifi_data)");
    }
  }
  else
  {
    trace_error("Can't initialize wifi scan - interface is NULL");
  }
...
bool init_netlink_channel(struct netlink_channel *channel, const char *interface)
{
  ...
  if(channel != NULL && interface != NULL)
  {
    channel->sequence = 1;
    ...
    if(channel->buf != NULL)
    {
      ...
      if(channel->ifindex != 0)
      {
        ...
        ret = init_netlink_socket(channel);

        if(!ret)
        {
          close_netlink_channel(channel);
        }
        else
        {
          ret = true;
      ...
      {
        perror("Incorrect network interface");
        free(channel->buf);
        channel->buf = NULL;
      }
    }
    ...
    {
      trace_error("Insufficent memory for netlink socket buffer");
    }
  }
  else if(channel == NULL)
  {
    trace_error("Can't initialize netlink channel - channel is NULL");
  }
  else if(interface == NULL)
  {
    trace_error("Can't initialize netlink channel - interface is NULL");
  }
...
bool init_netlink_socket(struct netlink_channel *channel)
{
  ...
  {
    trace_error("Can't initialize netlink socket - channel is NULL");
  }
...
int get_family_and_scan_ids(struct netlink_channel *channel)
{
  ...
  if(channel != NULL)
  {
    ...
    if(nlh != NULL)
    {
      ...
      {
        ret = receive_nl_message(channel, handle_CTRL_CMD_GETFAMILY);
      }
    }
    else
    {
      trace_error("Can't get scan ids - nl message is NULL");
    }
  }
  else
  {
    trace_error("Can't get scan ids - channel is NULL");
  }
  ...
  if(nlh != NULL && data != NULL)
  {
    ...
    if(genl != NULL)
    {
      ...
      if (tb[CTRL_ATTR_FAMILY_ID])
      {
        ...
        {
          if(parse_CTRL_ATTR_MCAST_GROUPS(tb[CTRL_ATTR_MCAST_GROUPS], channel))
          {
            ret = MNL_CB_OK;
          }
        }
      ...
      {
        perror("No family id attribute");
      }
    }
    else
    {
      trace_error("handle_CTRL_CMD_GETFAMILY error - netlink message payload pointer (genl) is NULL");
    }
  }
  else if(nlh == NULL)
  {
    trace_error("handle_CTRL_CMD_GETFAMILY error - nlh is NULL");
  }
  else
  {
    trace_error("handle_CTRL_CMD_GETFAMILY error - data is NULL");
  }
...
bool parse_CTRL_ATTR_MCAST_GROUPS(struct nlattr *nested, struct netlink_channel *channel)
{
    ...
    mnl_attr_for_each_nested(pos, nested)
    {
      ...
      if ( tb[CTRL_ATTR_MCAST_GRP_NAME])
      {
        ...
        if( strcmp(name, "scan") == 0 )
        {
          ...
          {
            perror("Missing id attribute for scan multicast group");
            ret = false;
          }
        }
      }
  ...
  {
    trace_error("parse_CTRL_ATTR_MCAST_GROUPS error - nested is NULL");
  }
  else
  {
    trace_error("parse_CTRL_ATTR_MCAST_GROUPS error - channel is NULL");
  }
...
bool subscribe_NL80211_MULTICAST_GROUP_SCAN(struct netlink_channel *channel, uint32_t scan_group_id)
{
  ...
  {
    trace_error("Can't subscribe multicast group for scanning - channel is NULL");
  }
...
bool read_past_notifications(struct netlink_channel *notifications)
{
  ...
  {
    result = set_channel_non_blocking(notifications);
    if(result)
    {
      int ret, run_ret;

      while( result && (ret = mnl_socket_recvfrom(notifications->nl, notifications->buf, MNL_SOCKET_BUFFER_SIZE) ) >= 0)
      {
        ...
        {
          result = false;
          perror("ReadPastNotificationsNonBlocking mnl_cb_run failed");
        }
      }
      ...
      if(ret == -1)
      {
        ...
        {
          result = false;
          perror("ReadPastNotificationsNonBlocking mnl_socket_recv failed");
        }
      }

      //no more notifications waiting
      result = set_channel_blocking(notifications);
    }
  }
  else
  {
    result = false;
    trace_error("Can't read past notifications - notifications netlink channel is NULL");
  }
...
bool set_channel_non_blocking(struct netlink_channel *channel)
{
  ...
  if(channel != NULL)
  {
    ...
    if(flags != -1)
    {
      ...
      {
        perror("SetChannelNonBlocking F_SETFL");
      }
    }
    else
    {
      perror("SetChannelNonBlocking F_GETFL");
    }
  }
  else
  {
    trace_error("Can't set channel as non-blocking - channel is NULL");
  }
...
bool set_channel_blocking(struct netlink_channel *channel)
{
  ...
  {
    trace_error("Can't set channel as blocking - channel is NULL");
  }
...
int trigger_scan(struct netlink_channel *channel)
{
  ...
  if(channel != NULL)
  {
    ...
    if(nlh != NULL)
    {
      ...
      if(send_nl_message(nlh, channel))
      {
        ret = receive_nl_message(channel, handle_NL80211_CMD_NEW_SCAN_RESULTS);
      }
    }
    else
    {
      trace_error("Can't trigger scan - nlh is NULL");
    }
  }
  else
  {
    trace_error("Can't trigger scan - channel is NULL");
  }
...
bool wait_for_new_scan_results(struct netlink_channel *notifications)
{
  ...
  if(notifications != NULL)
  {
    ...
    while(result && !scanning->new_scan_results)
    {
      if ( (ret = mnl_socket_recvfrom(notifications->nl, notifications->buf, MNL_SOCKET_BUFFER_SIZE)) <=0 )
      {
        result = false;
        perror("Waiting for new scan results failed - mnl_socket_recvfrom");
      }

        
      if ( result && (ret = mnl_cb_run(notifications->buf, ret, 0, 0, handle_NL80211_MULTICAST_GROUP_SCAN, notifications)) <=0 )
      {
        result = false;
        perror("Processing notificatoins failed - mnl_cb_run");
      }
    }
  }
  else
  {
    result = false;
    trace_error("Can't wait for new scan results - notifications netlink channel is NULL");
  }
...
int get_scan(struct netlink_channel *channel)
{
  ...
  if(channel != NULL)
  {
    ...
    {
      trace_error("Can't get scan - nlh is NULL)");
    }
  }
  else
  {
    trace_error("Can't get scan - channel is NULL");
  }
...
int get_station(struct netlink_channel *channel, uint8_t bssid[BSSID_LENGTH])
{
  ...
  if(channel != NULL)
  {
    ...
    if(nlh != NULL)
    {
      ...
      if(send_nl_message(nlh, channel))
      {
        ret = receive_nl_message(channel, handle_NL80211_CMD_NEW_STATION);
      }
    }
    else
    {
      trace_error("Can't get station - nlh is NULL");
    }
  }
  else
  {
    trace_error("Can't get station - channel is NULL");
  }
...
bool send_nl_message(struct nlmsghdr *nlh, struct netlink_channel *channel)
{
  ...
  {
    trace_error("Can't send nl message - nlh is NULL");
  }
  else
  {
    trace_error("Can't send nl message - channel is NULL");
  }
